...
export default async function AdminHomePage() {
  ...
  const matterRequests = await getMatterRequests()
  ...
  return (
    <main className="min-h-dvh bg-stone-50 text-stone-900">
      ...
      <div className="mx-auto max-w-5xl space-y-12 px-6 py-10">
        <section>
          <Link
            href="/corporate/admin/access-requests"
            className="border-brand-navy text-brand-navy hover:bg-brand-navy inline-flex rounded border bg-white px-4 py-2 text-sm font-semibold transition-colors hover:text-white"
          >
            Review portal access requests
          </Link>
        </section>
        ...
        <section>
          <div className="flex items-center justify-between">
            ...
            {isMock ? <span className="text-xs text-stone-400">Preview data</span> : null}
          </div>
          {matters.length === 0 ? (
                ...
                return (
                  <li key={key}>
                    <Link
                      ...
                      className="hover:border-brand-navy/30 flex items-center justify-between gap-4 rounded border border-stone-200 bg-white px-5 py-4 transition hover:shadow-sm"
                    >
                ...
                <li key={req.id} className="rounded border border-stone-200 bg-white px-5 py-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="mt-2 line-clamp-2 text-sm leading-6 text-stone-600">{req.description}</p>
                      {req.attachmentFilename ? (
                        <p className="mt-1 text-xs text-stone-400">Attachment: {req.attachmentFilename}</p>
                      ) : null}
                    </div>
                    <span
                      className={`shrink-0 rounded px-2 py-1 text-xs font-medium ${requestStatusBadge(req.status)}`}
                    >
                      {req.status}
                    </span>
                  </div>
